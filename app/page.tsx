...
const t = {
  ar: {
    ...
    nav: { about: "من نحن", projects: "المشاريع", payment: "السداد", map: "الخريطة", contact: "تواصل معنا", whatsapp: "واتساب" },
    hero: {
  ...
  en: {
    ...
    nav: { about: "About Us", projects: "Projects", payment: "Payment", map: "Map", contact: "Contact", whatsapp: "WhatsApp" },
    hero: {
    ...
    contact: {
      ...
      items: [
        ...
        { icon: "📍", label: "Address", value: "224 North 90th Street, New Cairo", href: "#" },
  ...
  return (
    <div className={`group rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border ${dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"}`}>
      <div className="relative overflow-hidden h-52">
        <img src={project.imgSrc} alt={project.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
        <div className="absolute bottom-4 right-4">
          <span className="bg-amber-500 text-white text-xs font-bold px-3 py-1 rounded-full">{project.down} {tr.projects.downLabel}</span>
        </div>
      </div>
      <div className="p-6">
        <div className="flex items-start justify-between mb-3">
          <div>
            <h3 className={`text-2xl font-black tracking-wide ${dark ? "text-amber-400" : "text-blue-900"}`}>{project.name}</h3>
            <p className={`text-sm mt-0.5 ${dark ? "text-gray-400" : "text-gray-500"}`}>{project.district}</p>
          </div>
          <span className={`text-xs font-semibold px-2 py-1 rounded-lg ${dark ? "bg-gray-700 text-amber-300" : "bg-blue-50 text-blue-900"}`}>{project.years}</span>
        </div>
...
export default function Home() {
  ...
  const navbarBg = scrolled ? (dark ? "bg-gray-900 shadow-md shadow-black/30 py-3" : "bg-white shadow-md py-3") : "bg-transparent py-5";
  ...
  return (
    <main className={`min-h-screen ${bg} transition-colors duration-300`} dir={tr.dir}>

      {/* Navbar */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${navbarBg}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Image src="/logo.png" alt="DAR EL ARKAN" width={50} height={50} className="rounded-lg" />
            <div>
              <h1 className={`font-black text-sm leading-none tracking-widest ${navText}`}>{tr.brand.name}</h1>
              <p className={`text-xs tracking-wider mt-0.5 ${scrolled ? "text-amber-500" : "text-amber-300"}`}>{tr.brand.sub}</p>
            </div>
          </div>
          ...
          <ul className="hidden md:flex items-center gap-6">
            {navLinks.map((l) => (
              <li key={l.href}>
                <a href={l.href} className={`text-sm font-semibold transition-colors hover:text-amber-400 ${navText}`}>{l.label}</a>
              </li>
            ))}
          </ul>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setLang(lang === "ar" ? "en" : "ar")}
              className={`hidden md:flex items-center text-xs font-bold px-3 py-2 rounded-lg border transition-all ${btnBase}`}>
              {lang === "ar" ? "EN" : "ع"}
            </button>
            <button onClick={() => setDark(!dark)}
              className={`hidden md:flex items-center justify-center w-9 h-9 rounded-lg border transition-all ${btnBase}`}
              aria-label="Toggle dark mode">
              {dark ? "☀️" : "🌙"}
            </button>
            <a href="https://wa.me/201152722626" target="_blank" rel="noopener noreferrer"
              className="hidden md:inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm font-bold px-5 py-2.5 rounded-xl transition-colors">
              {tr.nav.whatsapp}
            </a>
            <button className="md:hidden p-2" onClick={() => setMenuOpen(!menuOpen)}>
              {[0,1,2].map(i => (
                <div key={i} className={`w-6 h-0.5 ${i<2?"mb-1.5":""} ${scrolled ? (dark?"bg-white":"bg-blue-900") : "bg-white"}`} />
              ))}
            </button>
          </div>
        </div>
        ...
        {menuOpen && (
          <div className={`md:hidden px-6 py-4 flex flex-col gap-3 border-t ${dark ? "bg-gray-900 border-gray-700" : "bg-white border-gray-100"}`}>
            {navLinks.map((l) => (
              <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)}
                className={`font-semibold py-2 border-b ${dark ? "text-white border-gray-700" : "text-blue-900 border-gray-100"}`}>{l.label}</a>
            ))}
      ...
      <section className="relative min-h-screen flex items-center justify-center text-center px-6"
        style={{ background: dark?"linear-gradient(135deg,#050d1a 0%,#0e2240 50%,#050d1a 100%)":"linear-gradient(135deg,#0a1628 0%,#1e3a5f 50%,#0a1628 100%)" }}>
        ...
        <div className="relative max-w-4xl text-white z-10">
          <p className="text-amber-400 text-sm font-semibold tracking-[0.3em] uppercase mb-4">{tr.hero.location}</p>
          <h1 className="text-5xl md:text-8xl font-black leading-none tracking-tight">{tr.hero.title}</h1>
          <div className="flex items-center justify-center gap-4 my-4">
            <div className="h-px w-16 bg-amber-400" />
            <span className="text-amber-400 text-base tracking-[0.4em] font-light">{tr.hero.subtitle}</span>
            <div className="h-px w-16 bg-amber-400" />
          </div>
          <p className="mt-6 text-lg md:text-xl leading-relaxed text-gray-300 max-w-2xl mx-auto">
            {tr.hero.desc}<br />
            <span className="text-white font-semibold">{tr.hero.desc2}</span>
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4 mt-10">
            <a href="https://wa.me/201152722626" target="_blank" rel="noopener noreferrer"
              className="bg-green-600 hover:bg-green-500 px-8 py-4 rounded-2xl text-lg font-bold transition-all hover:scale-105 shadow-lg shadow-green-900/30">
              {tr.hero.cta1}
            </a>
            <a href="#projects"
              className="bg-white/10 hover:bg-white/20 border border-white/30 backdrop-blur-sm px-8 py-4 rounded-2xl text-lg font-bold transition-all hover:scale-105">
              {tr.hero.cta2}
            </a>
          </div>
        </div>
      ...
      {/* Stats */}
      <section className={`py-14 px-6 ${dark?"bg-gray-950":"bg-blue-900"}`}>
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-center text-white">
          {tr.stats.map((s) => (
            <div key={s.label}>
              <p className="text-4xl md:text-5xl font-black text-amber-400"><Counter target={s.value} suffix={s.suffix} /></p>
              <p className="mt-2 text-sm text-blue-200 font-medium">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* About */}
      <section id="about" className={`py-24 px-6 ${bgAlt}`}>
        <div className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-amber-500 text-sm font-semibold tracking-widest uppercase mb-3">{tr.about.tag}</p>
              <h2 className={`text-4xl font-black leading-tight ${tp}`}>{tr.about.title}</h2>
              <p className={`mt-6 leading-8 text-base ${ts}`}>{tr.about.body}</p>
            ...
            <div className="relative">
              <div className="aspect-square rounded-3xl overflow-hidden shadow-2xl flex items-center justify-center"
                ...
                <Image src="/logo.png" alt="DAR EL ARKAN" width={200} height={200} className="opacity-20" />
                <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center px-8">
                  <p className="text-6xl font-black text-amber-400">11+</p>
                  <p className="text-xl font-light mt-2">{tr.about.yrsLabel}</p>
                  <div className="mt-4 h-px w-16 bg-amber-400/50" />
                  <p className="mt-4 text-sm text-blue-200">{tr.about.chairman}</p>
                </div>
              </div>
              <div className="absolute -bottom-4 -right-4 bg-amber-500 text-white rounded-2xl px-5 py-3 shadow-xl">
      ...
      {/* Projects */}
      <section id="projects" className={`py-24 px-6 ${bg}`}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-amber-500 text-sm font-semibold tracking-widest uppercase mb-3">{tr.projects.tag}</p>
            <h2 className={`text-4xl font-black ${tp}`}>{tr.projects.title}</h2>
            <p className={`mt-4 max-w-xl mx-auto ${ts}`}>{tr.projects.desc}</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {tr.projectData.map((p) => <ProjectCard key={p.id} project={p} tr={tr} dark={dark} />)}
          </div>
        </div>
      </section>

      {/* Payment */}
      <section id="payment" className={`py-24 px-6 ${bgAlt}`}>
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-amber-500 text-sm font-semibold tracking-widest uppercase mb-3">{tr.payment.tag}</p>
            <h2 className={`text-4xl font-black ${tp}`}>{tr.payment.title}</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {tr.payment.plans.map((plan) => (
              <div key={plan.title}
                className={`rounded-3xl p-8 text-center shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg ${
      ...
      {/* Map */}
      <section id="map" className={`py-24 px-6 ${bg}`}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10">
            <p className="text-amber-500 text-sm font-semibold tracking-widest uppercase mb-3">{tr.map.tag}</p>
            <h2 className={`text-4xl font-black ${tp}`}>{tr.map.title}</h2>
            <p className={`mt-3 ${ts}`}>{tr.map.desc}</p>
          </div>
          <div className={`relative rounded-3xl overflow-hidden shadow-2xl border ${dark?"border-gray-700":"border-gray-100"}`}>
            <img src="/bait-elwatan-map.jpg" alt="Bayt Al-Watan Map" className="w-full" />
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 7680 10867" preserveAspectRatio="xMidYMid meet">
              {mapDistricts.map((d) => (
                <a key={d.href} href={d.href}>
                  ...
                  <polygon points={d.points} fill="rgba(30,58,95,0)" stroke="rgba(30,58,95,0)" strokeWidth="20"
      ...
      {/* Contact */}
      <section id="contact" className={`py-24 px-6 ${dark?"bg-gray-950":"bg-blue-900"} text-white`}>
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-amber-400 text-sm font-semibold tracking-widest uppercase mb-3">{tr.contact.tag}</p>
          <h2 className="text-4xl font-black mb-4">{tr.contact.title}</h2>
          <p className="text-blue-200 mb-10 max-w-xl mx-auto leading-7">{tr.contact.desc}</p>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <a href="https://wa.me/201152722626" target="_blank" rel="noopener noreferrer"
              className="bg-green-600 hover:bg-green-500 px-8 py-4 rounded-2xl font-bold text-lg transition-all hover:scale-105">
              {tr.contact.btnWa}
            </a>
            <a href="tel:01152722626" className="bg-white text-blue-900 hover:bg-blue-50 px-8 py-4 rounded-2xl font-bold text-lg transition-all hover:scale-105">
              {tr.contact.btnCall}
            </a>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className={`py-12 px-6 ${dark?"bg-black":"bg-gray-950"} text-white`}>
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-3 gap-8 mb-10">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <Image src="/logo.png" alt="DAR EL ARKAN" width={44} height={44} className="rounded-lg opacity-80" />
                <div>
            ...
            <div>
              <p className="font-bold text-sm mb-4 text-gray-300 tracking-widest uppercase">{tr.footer.quickLinks}</p>
              <ul className="space-y-2">
                {navLinks.map((l) => (
                  <li key={l.href}><a href={l.href} className="text-gray-400 hover:text-amber-400 text-sm transition-colors">{l.label}</a></li>
                ))}
              </ul>
            </div>
            <div>
              <p className="font-bold text-sm mb-4 text-gray-300 tracking-widest uppercase">{tr.footer.follow}</p>
              <div className="flex flex-col gap-3">
                {[
                  ...
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                    className={`${s.color} hover:opacity-90 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-all inline-block text-center`}>
                    {s.label}
                  </a>
                ))}
              </div>
